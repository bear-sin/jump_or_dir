...
void gl (int czas, int glos){ //播放声音

tone(3, glos);   //在3号角产生频率为glos的脉冲，驱动蜂鸣器
delay(czas);     //保持的时间
noTone(3);//关闭蜂鸣器
}
...
void poczatek(){    // 启动时播放音乐
  lcd.setCursor(2,0);//在第二列第一行开始显示
...
void menu() {//初始话时模式选择菜单
  
if (napis > 4 || napis < 0) {napis == 0;}
if (digitalRead(4) == HIGH) { napis++;}
  switch (napis) {
